...
//                    return $digest === crypt($password);
        ...
        public function addUser($email, $password, $type, $name, $surname)
        {
         ...
         public function addCourse($title, $description, $User_id)
        {
          
                $query = $this->db->prepare("INSERT IGNORE INTO course (title, description, creation_timestamp, User_id )  VALUES(?,?,?,?) ");
                return $query->execute(array($title, $description, time(), $User_id));
